...
You take a dependency, and on a hosted service you take a network call on the auth path, which you can cache. If keys are incidental to the product, use a service. If keys _are_ the product because you are building a gateway or an IAM system, build it.
...
// data.key is shown once. Store it on the client. Never log it server-side.
```
...
If you build this, use a CSPRNG, not `Math.random()`. Hash with something fast and keyed (SHA-256 plus a pepper). API keys are already high entropy, so bcrypt is the wrong tool. Index the hash. Keep the plaintext out of logs, errors, and analytics.
...
Cache valid keys for a short TTL. Use a fast hash. Put the counter in something built for distributed counts. Verify close to where the request lands. If the API is on the edge and the database is in one region, you will feel it.
...
| ------------------------------------------------------ | -------------------------------------------- | ---------------------------- | --------------------------- |
| Route and transform traffic                            | Yes                                          | Yes                          | No. Not a gateway.          |
...
If you control the prefix, customers never have to see this. You can reissue the same plaintext into the new store instead of forcing a rotation.
...
If your needs are basic quotas and you are already in that stack, start there. If you want keys as their own portable thing, on AWS or Cloudflare or a server you run, look at a dedicated service next to it, not instead of thinking.
